...
Parallel Program using Red-Black GS iterative method
Using delta = 0.01
    ...
    while(1)
    {
        ...
        for(i=1;i<local_rows-1;i++)
        {
            for(j=1;j<n-1;j++)
            {
                if((i+j+iter)%2)
                {
                    var = (loc_phi[i][j+1] + loc_phi[i][j-1] + loc_phi[i-1][j] + loc_phi[i+1][j] + q[i][j]*del*del)/4;
                    norm_err += (var-loc_phi[i][j])*(var-loc_phi[i][j]);
                    loc_phi[i][j] = var;
                }
            }
        }
        ...
        if(rank!=0 && rank!=size-1)
        {
            ...
            for(j=1;j<n-1;j++)
            {
                if((iter+i+j)%2)
                {
                    var = (loc_phi[i][j+1] + loc_phi[i][j-1] + loc_left[j] + loc_phi[i+1][j] + q[i][j]*del*del)/4;
                    norm_err += (var-loc_phi[i][j])*(var-loc_phi[i][j]);
                    loc_phi[i][j] = var;
                }
            }
            ...
            for(j=1;j<n-1;j++)
            {
                if((iter+i+j)%2)
                {
                    var = (loc_phi[i][j+1] + loc_phi[i][j-1] + loc_phi[i-1][j] + loc_right[j] + q[i][j]*del*del)/4;
                    norm_err += (var-loc_phi[i][j])*(var-loc_phi[i][j]);
                    loc_phi[i][j] = var;
                }
            }
        ...
        else if(rank==size-1)
        {
            ...
            for(j=1;j<n-1;j++)
            {
                if((iter+i+j)%2)
                {
                    var = (4*loc_phi[i-1][j] - loc_phi[i-2][j])/3.0;
                    norm_err += (var-loc_phi[i][j])*(var-loc_phi[i][j]);
                    loc_phi[i][j] = var;
                }
            }
